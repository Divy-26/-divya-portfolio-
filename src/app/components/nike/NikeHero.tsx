import { motion } from 'motion/react';

export default function NikeHero() {
  return (
    <section className="min-h-screen bg-[#0D0D0D] text-white relative overflow-hidden flex items-center justify-center">
      <div className="absolute inset-0 opacity-30">
        <motion.div
          animate={{
            x: [0, -100, 0],
            opacity: [0.3, 0.5, 0.3]
          }}
          transition={{
            duration: 8,
            repeat: Infinity,
            ease: "linear"
          }}
          className="absolute top-1/4 left-0 w-full h-32 bg-gradient-to-r from-transparent via-orange-500 to-transparent blur-3xl"
        />
        <motion.div
          animate={{
            x: [0, 100, 0],
            opacity: [0.2, 0.4, 0.2]
          }}
          transition={{
            duration: 10,
            repeat: Infinity,
            ease: "linear"
          }}
          className="absolute bottom-1/4 right-0 w-full h-32 bg-gradient-to-r from-transparent via-orange-600 to-transparent blur-3xl"
        />
      </div>

      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-40" />

      <div className="relative z-10 text-center px-6">
        <motion.div
          initial={{ opacity: 0, y: 40 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 1 }}
        >
          <motion.div
            animate={{ opacity: [1, 0.7, 1] }}
            transition={{ duration: 2, repeat: Infinity }}
            className="text-sm tracking-[0.3em] mb-8 text-orange-500"
          >
            DIGITAL CAMPAIGN 2026
          </motion.div>

          <h1 className="text-[10rem] md:text-[15rem] leading-none tracking-tighter mb-4">
            NIKE
          </h1>

          <motion.h2
            animate={{ x: [0, 10, 0] }}
            transition={{ duration: 3, repeat: Infinity }}
            className="text-[5rem] md:text-[8rem] leading-none tracking-tight mb-8"
          >
            AFTER DARK
          </motion.h2>

          <div className="text-2xl md:text-4xl tracking-wide text-orange-500">
            RUN THE CITY.
          </div>
        </motion.div>
      </div>

      <motion.div
        animate={{ y: [0, 10, 0] }}
        transition={{ duration: 2, repeat: Infinity }}
        className="absolute bottom-12 left-1/2 -translate-x-1/2"
      >
        <div className="w-px h-16 bg-gradient-to-b from-white to-transparent" />
      </motion.div>
    </section>
  );
}
